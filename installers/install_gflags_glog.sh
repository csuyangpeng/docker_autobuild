...
CXXFLAGS="-fPIC" cmake -DBUILD_SHARED_LIBS=ON ..
make -j8
make install
popd
...
# Clean up.
# The shared library need to be delete to avoid confusion to caffe glog
# But that can be avoid if we compile the caffe with static glog
